...
Read the `format` field from the manifest (`node .claude/skills/workflow-manifest/scripts/manifest.js get {topic} planning.format`). This determines which output format adapters to pass to the agent.
...
2. **Staging file path** — `.workflows/{topic}/implementation/review-tasks-c{cycle-number}.md`
