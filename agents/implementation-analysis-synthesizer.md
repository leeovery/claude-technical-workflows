...
1. **Read all findings files** from `.workflows/{work_unit}/implementation/` — look for `analysis-duplication-c{cycle-number}.md`, `analysis-standards-c{cycle-number}.md`, and `analysis-architecture-c{cycle-number}.md`
...
6. **Write report** — output to `.workflows/{work_unit}/implementation/analysis-report-c{cycle-number}.md`
7. **Write staging file** — if actionable tasks exist, write to `.workflows/{work_unit}/implementation/analysis-tasks-c{cycle-number}.md` with `status: pending` for each task
