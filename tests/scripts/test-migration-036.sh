#!/bin/bash
#
# Tests for migration 036: Backfill completed_at on completed work units
#
# Run: bash tests/scripts/test-migration-036.sh
#
...
MIGRATION="$REPO_DIR/skills/workflow-migrate/scripts/migrations/036-completed-at.sh"
...
setup() {
  TEST_DIR=$(mktemp -d /tmp/migration-036-test.XXXXXX)
...
# --- Test 1: Happy path — completed work unit gets backfilled ---
test_happy_path() {
  setup
...
JSON
  ...
  # Create an artifact file with a known mtime
...
# --- Test 2: Skip — already has completed_at ---
test_skip_existing() {
  setup
...
test_multiple() {
  ...
  # completed without completed_at — should be updated
...
JSON
  ...
  # in-progress — should NOT be updated
...
# --- Run all tests ---
echo "Running migration 036 tests..."
echo ""
