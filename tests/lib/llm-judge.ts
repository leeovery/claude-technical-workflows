...
export async function evaluateContent(
  ...
  return judge.evaluate(content, criteria, options.threshold || 0.8);
}

/**
 * Check if content satisfies all criteria (strict mode)
 */
export async function contentSatisfiesAll(
  content: string,
  criteria: string[],
  model: 'haiku' | 'sonnet' | 'opus' = 'haiku'
): Promise<boolean> {
  const result = await evaluateContent(content, criteria, {
    model,
    threshold: 1.0,
  });
...
/**
 * Check if content satisfies at least one criterion
 */
export async function contentSatisfiesAny(
  content: string,
  criteria: string[],
  model: 'haiku' | 'sonnet' | 'opus' = 'haiku'
): Promise<boolean> {
